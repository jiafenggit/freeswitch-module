...
SWITCH_DECLARE_GLOBAL_STRING_FUNC(set_global_db_column_call_times, globals.db_column_call_times);
...
static switch_status_t nibblebill_load_config(void)
{
	...
	if ((settings = switch_xml_child(cfg, "settings"))) {
		for (param = switch_xml_child(settings, "param"); param; param = param->next) {
			...
			} else if (!strcasecmp(var, "db_column_call_times")) {
                                set_global_db_column_call_times(val);
...
static switch_bool_t bill_event(double billamount, float billtime, const char *billaccount, int calltimes,switch_channel_t *channel)
{
	...
	if (globals.custom_sql_save) {
		...
		sql = dsql = switch_mprintf("UPDATE %s SET %s=%s-%f, %s=%s+%f, %s=%s-%f, %s=%s-%f, %s=%s+%d  WHERE %s='%s'", globals.db_table, 
									globals.db_column_cash,	globals.db_column_cash, billamount, 
									globals.db_column_call_duration, globals.db_column_call_duration, billtime,
									...
									globals.db_column_call_times, globals.db_column_call_times, calltimes,
...
static switch_status_t do_billing(switch_core_session_t *session)
{
	...
	if ((ts - nibble_data->lastts) >= 0) {
		...
		if (!(switch_strlen_zero(billincrement))) {
			...
			billtime = (float)chargedunits / 1000000 / 60;
		} else {		
		...
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "Billing $%f to %s (Call: %s / %f so far during %f)\n", billamount, billaccount,
						  uuid, nibble_data->total, billtime);
...
SWITCH_MODULE_SHUTDOWN_FUNCTION(mod_nibblebill_shutdown)
{
	...
	switch_safe_free(globals.db_column_call_times);
